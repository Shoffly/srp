...
st.write(f" This is the branch performance for the dates between {sd} and {ed}.")

tab1, tab2, tab3 = st.tabs(["CFC", "Mohandeseen", "Arkan"])

with tab1:
   st.header("CFC Performance")
    # Set the specific channel to filter
   Branch_to_filter = 'CFC'
   # Use columns layout manager
   col1, col2 = st.columns(2)
   # Filter the DataFrame based on the channel
   filtered_df_t = fil_df[fil_df['Branch'] == Branch_to_filter]
   # Revenue metric
   # Convert the 'Price' column to numeric data type
   ...
   branch_sales_t = filtered_df_t.groupby('Channel')['Sales'].sum()
   ...
   title = ax.set_title('Sales by Channel', fontsize=16, color='white')
   ...
   for label in labels:
       label.set_color('white')
   # Set title color to white
   title2 = ax.set_title('Salesmix by revenue', fontsize=16, color='white')
   ...
   for label in labels:
       label.set_color('white')
   # Set title color to white
   title = ax.set_title('Category Salesmix', fontsize=16, color='white')
...
with tab2:
   st.header("Mohandeseen Performance")
   # Set the specific channel to filter
   Branch_to_filter = 'Mohandeseen'
   # Use columns layout manager
   col1, col2 = st.columns(2)
   # Filter the DataFrame based on the channel
   filtered_df_t = fil_df[fil_df['Branch'] == Branch_to_filter]
...
with tab3:
   st.header("Arkan Performance")
   # Set the specific channel to filter
   Branch_to_filter = 'Arkan'
   # Use columns layout manager
   col1, col2 = st.columns(2)
   # Filter the DataFrame based on the channel
   filtered_df_t = fil_df[fil_df['Branch'] == Branch_to_filter]
