...
if authentication_status == True:
    ...
    Item = st.sidebar.selectbox("Pick the item", item_select)
    ...
    fil_df = df[(df['Date'] >= sd) & (df['Date'] <= ed) & (df['Items'].isin([Item]))]
    ...
    fil_df_c = df[(df['Date'] >= sd_c) & (df['Date'] <= ed_c) & (df['Items'].isin([Item]))]

    #page contents
    st.title(f"{Item} Analysis")
    # Display the value in the subheader using Markdown formatting
    st.write(f" This is a deep dive in {Item} for the dates between {sd.date()} and {ed.date()}.")
    ...
    st.write('''
    This item's sales dropped in the following branches
    ''')
    st.dataframe(nv_f)
    ...
    st.write('''
    This Item's sales increased or remained stable in the following branches
    ''')
    st.dataframe(pv_f)
    ...
    st.write('''
    This item's sales dropped in the following channels
    ''')
    st.dataframe(ncv_f)
    ...
    st.write('''
    This item's sales increased or remained stable in the following channels
    ''')
    st.dataframe(pvc_f)